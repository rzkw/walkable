...
<ArticleJsonLd
  title="My $0 Home Lab: Converting an old Windows 10 Laptop into a performance-optimised server"
  description="project turning a resource-constrained machine into a headless Ubuntu server"
  url="https://www.walk-llc.com/projects/old-laptop-server"
  date="2025-07-10"
  author="Rizky Ramadhani"
/>
...
### Can any old laptop be used as a home server?

Yes, most laptops from the last 10 years with at least 4GB of RAM and a 64-bit processor can run a Linux server. The key requirements are a working network interface and the ability to boot from USB.

### Do I need a static IP from my ISP?

No. Setting a static IP on your server within your local network is sufficient for home access. For external access, you can use a dynamic DNS service or Cloudflare Tunnel instead of a static public IP.

### How much does it cost to run a home server?

The server itself costs $0 if using existing hardware. Ongoing costs are limited to electricity — a lightweight headless laptop server typically draws 10-30W, costing roughly $10-30 per year depending on local power rates.
...
- [Automatic Server Updates - unattendedUpgrades with email notifications](https://www.ajfriesen.com/automatic-server-updates/)
